...
export const UserWithdrawalLimits = React.memo((props: UserWithdrawalLimitsProps) => {
    ...
    const currentUserWithdrawalLimitGroup = useMemo(() => withdrawLimit?.find(item => item.group === feeGroup.group) || withdrawLimit?.find(item => item.group === 'any'), [withdrawLimit, feeGroup]);

    const estimatedValueDay = useMemo(() => (+currentUserWithdrawalLimitGroup?.limit_24_hour - +usedWithdrawalLimitDay) / +price, [currentUserWithdrawalLimitGroup, price, usedWithdrawalLimitDay]);
    const estimatedValueMonth = useMemo(() => (+currentUserWithdrawalLimitGroup?.limit_1_month - +usedWithdrawalLimitMonth) / +price, [currentUserWithdrawalLimitGroup, usedWithdrawalLimitMonth, price]);
